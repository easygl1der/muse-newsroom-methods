    ...
    <div>
      ...
      <header className="site-header" id="masthead">
        <div className="headernav">
          <div className="header-row">
            ...
            <button className="icon-btn" type="button" aria-label="Search" onClick={() => setMobileOpen((value) => !value)}>
              <SearchIcon />
            </button>
            <button className="icon-btn icon-menu" type="button" aria-label="Open menu" aria-expanded={mobileOpen} onClick={() => setMobileOpen((value) => !value)}>
              {mobileOpen ? "✕" : "☰"}
            </button>
          </div>
        </div>
      ...
      <main className="page">
        ...
        <div className="meta-row">
          ...
          <button className="listen-btn" type="button" onClick={() => setListening((value) => !value)}>
            <Waveform />
        ...
        <div className="layout">
          <div>
            ...
            <section className="takeaways">
              ...
              <ul>
                ...
                <li>
                  ...
                  <a href={LINKS.security}>How We Built Safety Into Muse</a> and <a href={LINKS.designed}>How We Designed Muse</a>.
                </li>
              </ul>
            </section>
            ...
            <article className="article-prose">
              ...
              <p>
                It’s simple to use. People just tell Muse what needs to get done, and it takes action, powered by <a href={LINKS.spark}>Muse Spark</a>, Meta’s most capable model to date, built for real-world agentic work like this.
              </p>
              <h2>How It Works</h2>
              <p>
                Unlike other agents, <a href={LINKS.designed}>Muse was built to work for billions</a> of people worldwide, so there’s no learning curve. Anyone can use it out of the box, no technical experience required. It can handle tasks, like sending an email or booking travel, and it can take on big audacious goals. Once a person shares a goal with Muse, it helps them develop a personalized plan and coordinate their time and resources, then advances the work on its own.
              </p>
              <p>
                For tasks that take more time, Muse keeps working after people close the app, and comes back when something changes or when it needs approval, like before it sends an email or makes a purchase.
              </p>
              <p>
                When it comes time to pay, Muse can checkout with Link built by Stripe, and it is the first AI agent covered by <a href={LINKS.linkProtections}>Link’s purchase protections</a>.
              </p>
              <div className="carousel">
                {currentSlide ? (
                  <video key={currentSlide.id} src={currentSlide.src} controls playsInline aria-label={currentSlide.label} />
                ) : (
                  <div className="empty-state">No carousel media.</div>
                )}
                <div className="carousel-dots">
                  {CAROUSEL_SLIDES.map((item, index) => (
                    <button key={item.id} type="button" aria-label={item.label} aria-current={index === slide} onClick={() => setSlide(index)} />
                  ))}
                </div>
              </div>
              ...
              <p>
                Muse is rolling out in the US on iOS, Android, and <a href={LINKS.museAi}>muse.ai</a>, and coming soon to AI glasses. It’s free for most of what people need, with subscription plans for people who want to do more.
              </p>
              <p>
      ...
      {cookieOn ? (
        <div className="cookie-bar" id="GDPRConsentBar">
          ...
          <button type="button" onClick={() => setCookieOn(false)} style={{ marginLeft: 12, background: "transparent", color: "#fff", border: "1px solid #fff", padding: "4px 10px", cursor: "pointer" }}>
            Close
          </button>
        </div>
      ) : null}
